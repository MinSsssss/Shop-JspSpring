...
var isSupportTouch = 'ontouchstart' in window || navigator.MaxTouchPoints > 0 || navigator.msMaxTouchPoints > 0; // [workaround]
																													// IE
																													// doesn't
																													// have
																													// input
 ...
 *  - IE Compatibility Issue: http://goo.gl/sRLOAo - Scroll Issue:
 * http://goo.gl/sNjUc
 * 
 * @param {Rect}
 *            rect
...
function contains(array, item) {
  if (array && array.length && item) {
    if (array.indexOf) {
      ...
      // `DOMTokenList` doesn't implement `.indexOf`, but it implements
		// `.contains`
 ...
 * 
 * @param {Collection}
 *            collection - collection eg) node.childNodes, ...
 */
 ...
 *            array - array
 * @param {Function}
 *            fn - predicate function for cluster rule
 * @param {Array[]}
 */
...
/**
 * blank HTML for cursor position - [workaround] old IE only works with &nbsp; -
 ...
 *            node
 * @param {Function}
 *            [optional] pred - predicate function
 */
...
function nextPointWithEmptyNode(point, isSkipInnerOffset) {
  var node, offset; // if node is empty string node, return current node's
					// sibling.
 ...
 * @param {Boolean}
 *            [options.isDiscardEmptySplits] - default: false
 ...
 * 
 * assert if a node contains a "note-styletag" class, which implies that's a
 * custom-made style tag node
 * 
 * @param {Node}
 *            an HTML DOM node
 */
...
var Context_Context = /* #__PURE__ */function () {
  ...
  function Context($note, options) {
    ...
    this.modules = {};
    this.layoutInfo = {};
    this.options = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.extend(true, {}, options); // init
																															// ui
																															// with
																															// options
  ...
  _createClass(Context, [{
    ...
    value: function _initialize() {
      ...
      // set own id
      this.options.id = func.uniqueId(external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.now()); // set
																															// default
																															// container
																															// for
																															// tooltips,
      ...
      this.options.container = this.options.container || this.layoutInfo.editor; // add
																					// optional
																					// buttons
      ...
      var modules = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.extend({}, this.options.modules, external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.summernote.plugins || {}); // add
																																																										// and
																																																										// initialize
																																																										// modules
    ...
    /**
	 * Some buttons need to change their visual style immediately once they get
	 * pressed
	 */
...
external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.fn.extend({
  ...
  summernote: function summernote() {
    ...
    var options = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.extend({}, external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.summernote.options, hasInitOptions ? lists.head(arguments) : {}); // Update
																																																														// options
...
var range_WrappedRange = /* #__PURE__ */function () {
  function WrappedRange(sc, so, ec, eo) {
    ...
    this.isOnEditable = this.makeIsOn(dom.isEditable); // isOnList: judge
														// whether range is on
														// list node or not

    this.isOnList = this.makeIsOn(dom.isList); // isOnAnchor: judge whether
												// range is on anchor node or
												// not

    this.isOnAnchor = this.makeIsOn(dom.isAnchor); // isOnCell: judge whether
													// range is on cell node or
													// not

    this.isOnCell = this.makeIsOn(dom.isCell); // isOnData: judge whether range
												// is on data node or not
  ...
  range_createClass(WrappedRange, [{
    ...
    value: function normalize() {
      /**
		 ...
		 *            point
		 * @param {Boolean}
		 *            isLeftToRight - true: prefer to choose right node - false:
		 *            prefer to choose left node
		 * @return {BoundaryPoint}
		 */
      var getVisiblePoint = function getVisiblePoint(point, isLeftToRight) {
        ...
        // - case 02. if the point is on the right edge and prefer to choose
		// left node
        // - case 03. if the point is on the left edge and prefer to choose
		// right node
        // - case 04. if the point is on the right edge and prefer to choose
		// right node but the node is void
        // - case 05. if the point is on the left edge and prefer to choose left
		// node but the node is void
    ...
    value: function nodes(pred, options) {
      ...
      var fullyContains = options && options.fullyContains; // TODO compare
															// points and sort
    ...
    value: function wrapBodyInlineWithPara() {
      ...
      /**
		 * [workaround] firefox often create range on not visible point. so
		 * normalize here. - firefox: |
		 * <p>
		 * text
		 * </p>| - chrome:
		 * <p>
      ...
      if (topAncestor) {
        ...
        inlineSiblings = inlineSiblings.concat(dom.listNext(topAncestor.nextSibling, dom.isParaInline)); // wrap
																											// with
																											// paragraph
	 ...
	 * 
	 * @param {Boolean}
	 *            [findAfter] - find after cursor, default: false
	 * @return {WrappedRange}
	 */
	 ...
	 * example: range: 'hi
	 * 
	 * @Peter Pan' regex: '/@[a-z ]+/i' return range: '@Peter Pan'
	 * 
	 * @param {RegExp}
	 *            [regex]
...
/**
 * Data structure * BoundaryPoint: a point of dom tree * BoundaryPoints: two
 * boundaryPoints corresponding to the start and the end of the Range
 * 
 * See to
 * http://www.w3.org/TR/DOM-Level-2-Traversal-Range/ranges.html#Level-2-Range-Position
 */
...
/* harmony default export */ var range = ({
  ...
  createFromSelection: function createFromSelection() {
    ...
    if (env.isW3CRangeSupport) {
      ...
      var endPoint = textRangeToPoint(textRangeEnd, false); // same visible
															// point case: range
															// was collapsed.
  ...
  createFromNode: function createFromNode(node) {
    ...
    var so = 0;
    var ec = node;
    var eo = dom.nodeLength(ec); // browsers can't target a picture or void
									// node
...
var History_History = /* #__PURE__ */function () {
  ...
  History_createClass(History, [{
    ...
    /**
	 * @method rewind Rewinds the history stack back to the first snapshot
	 *         taken. Leaves the stack intact, so that "Redo" can still be used.
	 */
    ...
    /**
	 * @method commit Resets history stack, but keeps current editor's content.
	 */
    ...
    /**
	 * @method reset Resets the history stack completely; reverting to an empty
	 *         editor.
	 */
    ...
    value: function recordUndo() {
      ...
      this.stack.push(this.makeSnapshot()); // If the stack size reachs to the
											// limit, then slice it
...
var Style_Style = /* #__PURE__ */function () {
  ...
  Style_createClass(Style, [{
	 ...
	 * @method jQueryCSS
	 * 
	 * [workaround] for old jQuery passing an array of style properties to
	 * .css() will result in an object of property-value pairs. (compability
	 * with version < 1.9)
	 * 
	 * @private
	 * @param {jQuery}
	 ...
	 *            rng
	 * @param {Object}
	 *            [options] - options for styleNodes
	 * @param {String}
	 *            [options.nodeName] - default: `SPAN`
	 * @param {Boolean}
	 *            [options.expandClosestSibling] - default: `false`
	 * @param {Boolean}
	 *            [options.onlyPartialContains] - default: `false`
	 * @return {Node[]}
	 */
    ...
    value: function current(rng) {
      ...
      var styleInfo = this.fromNode($cont); // document.queryCommandState for
											// toggle state
      // [workaround] prevent Firefox nsresult: "0x80004005
		// (NS_ERROR_FAILURE)"
...
var Bullet_Bullet = /* #__PURE__ */function () {
  ...
  Bullet_createClass(Bullet, [{
    ...
    value: function wrapList(paras, listName) {
      ...
      var listNode = prevList || dom.insertAfter(dom.create(listName || 'UL'), last); // P to
																						// LI
    ...
    value: function releaseList(clustereds, isEscapseToBody) {
      ...
      external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.each(clustereds, function (idx, paras) {
        ...
        if (headList.parentNode.nodeName === 'LI') {
          ...
          paras = isEscapseToBody ? dom.listDescendant(middleList, dom.isLi) : lists.from(middleList.childNodes).filter(dom.isLi); // LI
																																	// to P
	 ...
	 * 
	 * Appends list to previous list item, if none exist it wraps the list in a
	 * new list item.
	 * 
	 * @param {HTMLNode}
	 *            ListItem
...
var Typing_Typing = /* #__PURE__ */function () {
  ...
  Typing_createClass(Typing, [{
	 ...
	 * 
	 * blockquoteBreakingLevel 0 - No break, the new paragraph remains inside
	 * the quote 1 - Break the first blockquote in the ancestors list 2 - Break
	 * all blockquotes, so that the new paragraph is not quoted (this is the
	 * default)
	 */
    ...
    value: function insertParagraph(editable, rng) {
      ...
      rng = rng.deleteContents(); // Wrap range if it needs to be wrapped by
									// paragraph
      ...
      if (splitRoot) {
        ...
        if (dom.isLi(splitRoot) && (dom.isEmpty(splitRoot) || dom.deepestChildIsEmpty(splitRoot))) {
          ...
          if (blockquote) {
            ...
            nextPara = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(dom.emptyPara)[0]; // If
																														// the
																														// split
																														// is
																														// right
            ...
            if (split) {
              ...
              dom.insertAfter(nextPara, blockquote); // There's no split if
														// we were at the end of
														// the blockquote
            }
          } else {
            nextPara = dom.splitTree(splitRoot, rng.getStartPoint()); // not a
																		// blockquote,
																		// just
																		// insert
																		// the
 ...
 * @param {object}
 *            startPoint Cell selected to apply change.
 * @param {enum}
 *            where Where change will be applied Row or Col. Use enum:
 *            TableResultAction.where
 * @param {enum}
 *            action Action to be applied. Use enum:
 *            TableResultAction.requestAction
 * @param {object}
 *            domTable Dom element of table to make changes.
 */
...
var TableResultAction = function TableResultAction(startPoint, where, action, domTable) {
  ...
  /**
	 ...
	 * 
	 * @param {int}
	 *            rowIndex Index position in line of virtual table.
	 * @param {int}
	 *            cellIndex Index position in column of virtual table.
	 * @param {object}
	 *            baseRow Row affected by this position.
	 * @param {object}
	 *            baseCell Cell affected by this position.
	 * @param {bool}
	 *            isSpan Inform if it is an span cell/row.
	 */
  ...
  /**
	 ...
	 * 
	 * @param {object}
	 *            virtualTableCellObj Object of specific position on virtual
	 *            table.
	 * @param {enum}
	 *            resultAction Action to be applied in that item.
	 */
  ...
  /**
	 ...
	 * 
	 * @param {int}
	 *            rowIndex Index of row to find free space.
	 * @param {int}
	 *            cellIndex Index of cell to find free space in table.
	 */
  ...
  /**
	 ...
	 * @param {object}
	 *            cell Cell to recover information.
	 */
  ...
  function addCellInfoToVirtual(row, cell) {
    ...
    setVirtualTablePosition(row.rowIndex, cellIndex, row, cell, cellHasRowspan, cellHasColspan, false); // Add
																										// span
																										// rows
																										// to
																										// virtual
...
var Table_Table = /* #__PURE__ */function () {
  ...
  Table_createClass(Table, [{
	 ...
	 * 
	 * @param {object} Element to recover attributes. @return {string} Copied
	 * string elements.
	 */
...
var Editor_Editor = /* #__PURE__ */function () {
  function Editor(context) {
    ...
    this.context.memo('help.fontName', this.lang.help.fontName); // native
																	// commands(with
																	// execCommand),
																	// generate
																	// function
    ...
    this.createLink = this.wrapCommand(function (linkInfo) {
      ...
      var isTextChanged = rng.toString() !== linkText; // handle spaced urls
														// from input
	 ...
	 * 
	 * @param {Object}
	 *            sObjColor color code
	 * @param {String}
	 *            sObjColor.foreColor foreground color
	 * @param {String}
	 *            sObjColor.backColor background color
	 */
	 ...
	 * 
	 * @param {String}
	 *            colorCode foreground color code
	 */
    ...
    /**
	 * remove media object and Figure Elements if media object is img with
	 * Figure.
	 */
  ...
  Editor_createClass(Editor, [{
    ...
    value: function initialize() {
      ...
      this.$editable.on('keydown', function (event) {
        ...
        _this2.context.triggerEvent('keydown', event); // keep a snapshot to
														// limit text on input
														// event
	 ...
	 * set the last range
	 * 
	 * if given rng is exist, set rng as the last range or create a new range at
	 ...
	 * get the last range
	 * 
	 * if there is a saved last range, return it or create a new range and
	 * return it
	 * 
	 * @return {WrappedRange}
	 */
    ...
    value: function beforeCommand() {
      this.context.triggerEvent('before.command', this.$editable.html()); // Set
																			// styleWithCSS
																			// before
																			// run
																			// a
      ...
      document.execCommand('styleWithCSS', false, this.options.styleWithCSS); // keep
																				// focus
																				// on
																				// editable
																				// before
    ...
    value: function insertImagesOrCallback(files) {
      ...
      if (callbacks.onImageUpload) {
        this.context.triggerEvent('image.upload', files); // else insert Image
															// as dataURL
      } else {
    ...
    value: function onFormatBlock(tagName, $target) {
      ...
      document.execCommand('FormatBlock', false, env.isMSIE ? '<' + tagName + '>' : tagName); // support
																								// custom
																								// class
    ...
    value: function fontStyling(target, value) {
      ...
      if (rng !== '') {
        ...
        external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(spans).css(target, value); // [workaround]
																												// added
																												// styled
																												// bogus
																												// span
    ...
    /**
	 * Removes all contents and restores the editable instance to an
	 * _emptyPara_.
	 */
  ...
  Dropzone_createClass(Dropzone, [{
    ...
    value: function attachDragAndDropEvent() {
      ...
      this.documentEventHandlers.onDragleave = function (e) {
        collection = collection.not(e.target); // If nodeName is BODY, then
												// just make it over (fix for
												// IE)
      ...
      // -but only if the editor is visible, i.e. has a positive width and
		// height


      this.$eventListener.on('dragenter', this.documentEventHandlers.onDragenter).on('dragleave', this.documentEventHandlers.onDragleave).on('drop', this.documentEventHandlers.onDrop); // change
																																															// dropzone's
																																															// message
																																															// on
																																															// hover.
      ...
      this.$dropzone.on('drop', function (event) {
        var dataTransfer = event.originalEvent.dataTransfer; // stop the
																// browser from
																// opening the
																// dropped
																// content
  ...
  Codeview_createClass(CodeView, [{
    ...
    value: function purify(value) {
      if (this.options.codeviewFilter) {
        ...
        value = value.replace(this.options.codeviewFilterRegex, ''); // allow
																		// specific
																		// iframe
																		// tag
    ...
    value: function activate() {
      ...
      if (CodeMirror) {
        var cmEditor = CodeMirror.fromTextArea(this.$codable[0], this.options.codemirror); // CodeMirror
																							// TernServer
    ...
    value: function deactivate() {
      var CodeMirror = this.CodeMirrorConstructor; // deactivate CodeMirror as
													// codable
...
var AutoReplace_AutoReplace = /* #__PURE__ */function () {
  ...
  AutoReplace_createClass(AutoReplace, [{
    ...
    key: "handleKeydown",
    value: function handleKeydown(e) {
      // this forces it to remember the last whole word, even if multiple
		// termination keys are pressed
...
var Buttons_Buttons = /* #__PURE__ */function () {
  ...
  Buttons_createClass(Buttons, [{
    ...
    value: function colorPalette(className, tooltip, backColor, foreColor) {
      ...
      return this.ui.buttonGroup({
        ...
        children: [this.button({
          ...
          items: (backColor ? ['<div class="note-palette">', '<div class="note-palette-title">' + this.lang.color.background + '</div>', '<div>', '<button type="button" class="note-color-reset btn btn-light btn-default" data-event="backColor" data-value="transparent">', this.lang.color.transparent, '</button>', '</div>', '<div class="note-holder" data-event="backColor"><!-- back colors --></div>', '<div>', '<button type="button" class="note-color-select btn btn-light btn-default" data-event="openPalette" data-value="backColorPicker">', this.lang.color.cpSelect, '</button>', '<input type="color" id="backColorPicker" class="note-btn note-color-select-btn" value="' + this.options.colorButton.backColor + '" data-event="backColorPalette">', '</div>', '<div class="note-holder-custom" id="backColorPalette" data-event="backColor"></div>', '</div>'].join('') : '') + (foreColor ? ['<div class="note-palette">', '<div class="note-palette-title">' + this.lang.color.foreground + '</div>', '<div>', '<button type="button" class="note-color-reset btn btn-light btn-default" data-event="removeFormat" data-value="foreColor">', this.lang.color.resetToDefault, '</button>', '</div>', '<div class="note-holder" data-event="foreColor"><!-- fore colors --></div>', '<div>', '<button type="button" class="note-color-select btn btn-light btn-default" data-event="openPalette" data-value="foreColorPicker">', this.lang.color.cpSelect, '</button>', '<input type="color" id="foreColorPicker" class="note-btn note-color-select-btn" value="' + this.options.colorButton.foreColor + '" data-event="foreColorPalette">', '</div>', // Fix
																																																																																																																																																																																																																																																																																																																																																																																																																				// missing
																																																																																																																																																																																																																																																																																																																																																																																																																				// Div,
																																																																																																																																																																																																																																																																																																																																																																																																																				// Commented
																																																																																																																																																																																																																																																																																																																																																																																																																				// to
          ...
          click: function click(event) {
            ...
            if (eventName === 'openPalette') {
              ...
              var $palette = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()($parent.find('#' + $picker.data('event')).find('.note-color-row')[0]); // Shift
																																												// palette
																																												// chips

              var $chip = $palette.find('.note-color-btn').last().detach(); // Set
																			// chip
																			// attributes
    ...
    /**
	 * image: [ ['imageResize', ['resizeFull', 'resizeHalf', 'resizeQuarter',
	 * 'resizeNone']], ['float', ['floatLeft', 'floatRight', 'floatNone']],
	 * ['remove', ['removeMedia']], ],
	 */
    ...
    /**
	 * table : [ ['add', ['addRowDown', 'addRowUp', 'addColLeft',
	 * 'addColRight']], ['delete', ['deleteRow', 'deleteCol', 'deleteTable']] ],
	 */
    ...
    value: function updateCurrentStyle($container) {
      ...
      if (styleInfo['font-family']) {
        ...
        $cont.find('.dropdown-fontname a').each(function (idx, item) {
          var $item = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(item); // always
																										// compare
																										// string
																										// to
																										// avoid
    ...
    value: function tableMoveHandler(event) {
      var PX_PER_EM = 18;
      var $picker = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(event.target.parentNode); // target
																															// is
																															// mousecatcher
...
var Toolbar_Toolbar = /* #__PURE__ */function () {
  ...
  Toolbar_createClass(Toolbar, [{
    ...
    value: function followScroll() {
      ...
      var statusbarHeight = this.$statusbar.height(); // check if the web app
														// is currently using
														// another static bar
    ...
    value: function showImageDialog() {
      ...
      return external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.Deferred(function (deferred) {
        ...
        _this2.ui.onDialogShown(_this2.$dialog, function () {
          _this2.context.triggerEvent('dialog.shown'); // Cloning imageInput to
														// clear element.
...
/**
 * Image popover module mouse events that show/hide popover will be handled by
 * Handle.js. Handle.js will receive the events and invoke
 * 'imagePopover.update'.
 */
...
var TablePopover_TablePopover = /* #__PURE__ */function () {
  ...
  TablePopover_createClass(TablePopover, [{
    ...
    value: function initialize() {
      ...
      var $content = this.$popover.find('.popover-content,.note-popover-content');
      this.context.invoke('buttons.build', $content, this.options.popover.table); // [workaround]
																					// Disable
																					// Firefox's
																					// default
																					// table
...
var AirPopover_AirPopover = /* #__PURE__ */function () {
  ...
  AirPopover_createClass(AirPopover, [{
    ...
    value: function initialize() {
      ...
      this.context.invoke('buttons.build', $content, this.options.popover.air); // disable
																				// hiding
																				// this
																				// popover
																				// preemptively
      ...
      }); // (re-)enable hiding after 'summernote.blur' has been handled (aka.
			// ignored).
    ...
    value: function replace() {
      ...
      if ($item.length) {
        var node = this.nodeFromItem($item); // If matchingWord length = 0 ->
												// capture OK / open hint / but
												// as mention capture "" (\w*)

        if (this.matchingWord !== null && this.matchingWord.length === 0) {
          this.lastWordRange.so = this.lastWordRange.eo; // Else si > 0 and
															// normal case ->
															// adjust range
															// "before" for
															// correct position
      ...
      if (!lists.contains([core_key.code.ENTER, core_key.code.UP, core_key.code.DOWN], e.keyCode)) {
        ...
        if (this.hints.length && keyword) {
          ...
          if (bnd) {
            ...
            this.$content.find('.note-hint-item:first').addClass('active'); // set
																			// position
																			// for
																			// popover
																			// after
...
var TooltipUI_TooltipUI = /* #__PURE__ */function () {
  function TooltipUI($node, options) {
    ...
    this.$tooltip = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(['<div class="note-tooltip">', '<div class="note-tooltip-arrow"></div>', '<div class="note-tooltip-content"></div>', '</div>'].join('')); // define
																																																											// event
